...
async function getBlog(slug: string) {
  ...
  try {
    const blog = await Blog.findOne({ slug })
      ...
      .populate("category", "name slug") // Ensure category is populated for breadcrumbs
      .lean();
    return blog;
  } catch (error) {
      ...
      <div className="bg-foreground/20 w-full pt-20 pb-32 px-4 border-b">
        <div className="max-w-4xl mx-auto text-center space-y-6">
            ...
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbItem>
                  ...
                  >
                    {blog.category?.name || categorySlug}
                  </BreadcrumbLink>
                </BreadcrumbItem>
                <BreadcrumbSeparator />
                <BreadcrumbItem>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 text-muted-foreground text-sm border-y py-4 w-fit mx-auto px-6">
            ...
            <ShareButtons title={blog.title} url={`/${categorySlug}/${slug}`} />
          </div>
        </div>
      </div>
